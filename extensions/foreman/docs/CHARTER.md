...
| **developer** | `openai-codex/gpt-5.5:xhigh` | read/write/edit/bash/grep/find/ls + `escalate_question` | Implements backend/logic and tests on disk; escalates real forks (can't ask the founder directly) | Judges its own work; calls `AskUserQuestion`/`foreman` (would hang the loop) |
| **ui-developer** | `cliproxy/gemini-3.5-flash-low:high` → `cliproxy/claude-opus-4-8:xhigh` fallback | read/write/edit/bash/grep/find/ls + `escalate_question` | Implements frontend/UI work with taste on `track:"frontend"`; escalates real forks | Judges its own work; calls `AskUserQuestion`/`foreman` |
...
4. **Implement**: the selected implementer makes the smallest scoped change and records a DEV-JSON handoff. `backend` routes to `developer`; `frontend` routes to `ui-developer` with same-round Opus fallback on tool failure. The implementer runs headless and cannot ask the founder; when a genuine fork blocks it, it calls `escalate_question` (recorded to the ledger, returns immediately) and stops — it never blocks on an interactive dialog.
4b. **Crew decision relay (`awaiting_decision`)**: if the implementer escalated a question, the loop PAUSES instead of hanging and surfaces it to the CTO. The CTO answers from context when it reasonably can, otherwise relays to the founder via `AskUserQuestion`, then resumes with `foreman({ resume: true, answer: "<decision>" })` (injected into the next round) or `foreman({ resume: true, reject: "<new direction>" })`. The pending question persists in the ledger so it survives a restart.
